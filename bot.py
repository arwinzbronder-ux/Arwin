...
async def rg_remove_id(interaction: discord.Interaction, friend_code: str):
    ...
    if found_user_id:
        ...
        save_data(data)
        sync_to_github(data)
        await interaction.response.send_message(f"🗑️ Removed ID `{friend_code}` from the list and set user to Offline.", ephemeral=False)
    else:
        await interaction.response.send_message(f"❌ ID `{friend_code}` not found in database.", ephemeral=True)
...
async def rg_tempban(interaction: discord.Interaction, member: discord.Member):
    ...
    if channel:
        ...
        await interaction.response.send_message(f"🚫 Banned {member.mention} from {channel.mention} for 48 hours.", ephemeral=False)
    else:
        await interaction.response.send_message(f"⚠️ Could not find channel `{CHECKIN_CHANNEL_NAME}` to apply ban.", ephemeral=True)
